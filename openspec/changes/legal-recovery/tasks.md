...
- [x] BE `DataExportResponse` (profile / shelters records —
      phone null for the admin, address null for USER rows; the review half was
      dropped with the review model, `V21__drop_reviews.sql`)
- [x] `AccountService.dataExport`: decrypted profile + verified levels,
      every author-scoped shelter row (all statuses,
      `ShelterRepository.findByCreatedBy`) — the authoritative batched shelter
      read; the removed `myReviews`/review half of the export went with the
      review model (`V21__drop_reviews.sql`)
...
- [x] `AccountDataExportIT`: anonymous 401; decrypted profile + own
      shelter (fields incl. review status, null address); author scoping
      (other user's rows absent); empty lists without contributions
- [x] FE `DataExportResponse`/`DataExportShelter`
