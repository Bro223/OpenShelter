...
- [ ] `shared/location-input.ts`: `parseLocationInput` + `ParseLocationResult`
...
- [ ] `shared/location-input.spec.ts`: ≥30 cases (every pattern, DMS,
      separators, labels, swap, out-of-bounds, garbage, empty)
- [ ] `gateways/geo-gateway.ts` (+spec): `resolve(url): Promise<{latitude, longitude}>`
      → `POST /api/geo/resolve`, error via `ApiError`
- [ ] `core/models.ts`: `LocationResolved { latitude, longitude }` (match
      backend field names exactly)
- [ ] `submit-shelter-page.*`: location section rework — smart input +
...
- [ ] short-link detection (`maps.app.goo.gl`) routes through
      `geo-gateway.resolve()` with a pending state on the button
- [ ] page spec updates: new capture modes, swap hint, error mapping,
      geolocation mock (vi.stubGlobal), short-link path mock
- [ ] docs sync: `frontend/docs/agent/02-CONTEXT-API.md` (+ endpoint row +
...
- [ ] `api/LocationResolveRequest.java` (record, `@Size(max=2048) url`,
      must parse as http/https URL) + `api/LocationResolvedDto.java`
      (`latitude`, `longitude` — match FE model)
- [ ] `app/MapsUrlCoordinates.java` — URL → pair extraction (same pattern
      list as FE; bbox via `GeoPoint.inEstonia`; same auto-swap rule)
- [ ] `app/LocationResolveService.java` — host whitelist
...
- [ ] `api/LocationController.java` — `POST /api/geo/resolve`, JWT
      (inside the existing authenticated set — verify SecurityConfig and
      add to it, NOT permitAll), status mapping
- [ ] SecurityConfig: per-IP token bucket 5/60s for `/api/geo/resolve`
      (pattern of the existing buckets; property in application.yml)
- [ ] tests: `MapsUrlCoordinatesTest` (same fixture table as the FE spec
...
- [ ] docs sync: `context-and-tasks/agent/06-CONTEXT-API.md` (+ endpoint),
...
- [ ] `mvn -q test` · `cd frontend && npx ng test --watch=false` · both
      `tsc --noEmit` configs · prettier on touched files
- [ ] live E2E: backend up (dev profile), submit page — paste coordinate
