...
to the login page with a success hint. A rejected code SHALL
surface as an inline banner using generic copy that never echoes backend internals. The page SHALL
no longer read a `token` query parameter.
