...
The map page SHALL keep the map and the sidebar list synchronized: selecting a list row flies the
map to that shelter, and selecting a marker highlights the matching list row.

#### Scenario: Row selected flies map

- **WHEN** the user selects a sidebar row
- **THEN** the map view centers on that shelter's coordinates

#### Scenario: Marker selected highlights row

- **WHEN** the user clicks a shelter marker on the map
- **THEN** the corresponding sidebar row becomes visibly selected

### Requirement: Navigate to shelter detail

The map page SHALL navigate to `/shelters/{id}` when the user clicks a map marker or a sidebar row.

#### Scenario: Marker click navigates

- **WHEN** the user clicks a shelter marker
