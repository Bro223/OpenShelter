# legal-recovery Specification

## Purpose

TBD - created by archiving change legal-recovery. Update Purpose after archive.
...
scheduled job that enforces them is a deployment switch (`RETENTION_ENABLED`)
and is off in the development setup, where the data is therefore kept.
...
- **THEN** the policy renders with the sections: what we collect, why we
  ...
  providers only), self-service rights, retention (24-month horizons,
  enforced by the config-gated job) and contact/changes
