...
- `DevRegistryClient` returns the fixture; `CsvRegistryClient` logic tested against a mock
  HTTP layer (`CsvRegistryClientTest`: download/parse, Last-Modified/ETag versioning +
  304 not-modified, If-Modified-Since from the previous audit row, transient retry vs
  4xx fail-fast, axis-order detection from values).
