...
- **Shelter reports (V9, D1):** `POST /api/shelters/{id}/reports` body
  ...
  (stored only for OTHER)}` → **200** `{"damped": bool}`. Check order (each an `ErrorResponse`): **401** no/invalid
  ...
  report throttle ("Too many report requests"). **V11 (community-review-queue v2, three
  distinct confirmers since community-self-moderation):** an `OPEN_CONFIRMED` report —
  and a live `OPEN` tap — on a USER row that is still `review_status = NEW`
  recomputes the row's tally of distinct community confirmers (verified
  users other than the submitter with an open (non-dismissed) `OPEN_CONFIRMED`
  report or a current OPEN tap, each counted once), and at three the row
  is promoted to `CONFIRMED` IN THE SAME TRANSACTION (audited `AUTO_CONFIRM`,
  the crossing user as actor); the submitter's own positive action never
  promotes, and registry / already confirmed rows are untouched. The derived reported state appears on the next
...
- **Auto-hide (V9, D1, trust-weighted since community-self-moderation):** on
  the insert that brings a shelter's trust-weighted `NON_EXISTENT` hide
  tally — the sum of the distinct reporters' derived weights, dampened
  reports contributing 0, admin-dismissed reports excluded — from below
  5 to at least 5, an `ACTIVE` shelter whose `autoHideDisarmed` is still `false` becomes
  `INACTIVE` (soft auto-hide — the row and its reports are retained; it drops out of
  the public list and the map; five baseline reporters — the 5th report). The trigger fires
  **only on that one crossing insert**: after any
  manual status change (the admin restore — which sets the disarm flag — or the author's delete),
  the tally is already at or above 5 (or the flag is set), so later reports increment it but never
  re-hide. No other path auto-hides; a below-threshold tally only flags (the orange "Reported" state
  on the still-active shelter).
- **Status flag net (V9, D1 — retired):** `CLOSED` and `OPEN_CONFIRMED` netted to a **display-only** flag (V9-era derivation — the live equivalent is the V22 `openStatus` block, `state` OPEN/CLOSED, fresh ≤ 2 h; computed at read time, never stored, never status): `closed >
...
- **Admin moderation (`/admin/*`, admin-moderation D3/D4)** — the trust layer's human lever.
  ...
  array; all writes answer **204** (no body) EXCEPT `POST /admin/shelters/{id}/review` (200 `{"ok": true}`), and are single-row transactions — no bulk
...
1. **Community reports + confirmation IS the moderation** — no moderator role anywhere in the
   system, and no star rating. Rows publish as `NEW` and reach `CONFIRMED` automatically (three
   distinct non-submitter confirmations — open `OPEN_CONFIRMED` reports and current OPEN taps,
   each counted once — at threshold 3, audited `AUTO_CONFIRM`, the crossing user as actor)
   or via the rare admin CONFIRM; admin REJECT (reason required) hides the row via
   `status = INACTIVE`.
2. **Deferred (documented, not built):** a server-side NEAREST ranking — `GET
   /api/shelters/nearest` — needs `GeoService` + PostGIS GIST index (the bbox box filter +
   `limit`/`offset` paging ARE shipped — shelter-bbox-paging, plain composite B-tree, no
   PostGIS). Add notes/`TODO` in the controller, do not implement.
...
4. **Trust state is server-derived (V9, shelter-trust-and-reports)** — the reported state
   (`nonexistentReports`, the `openStatus` block), the fresh occupancy block and the trust
   ...
   (exactly-once weighted-tally crossing, D1) and the report's `dismissed_at` stamp (V10 — once, idempotent, never deletes) —
   plus, V11 (community-review-queue v2), the `shelters.review_status` transitions (the
   three-distinct-confirmer NEW→CONFIRMED promotion in the crossing action's transaction, admin
   CONFIRM/REJECT, the restore's REJECTED→NEW) and the matching `moderation_actions` audit
   row, written in the SAME transaction as each action (append-only; the UI's "Newly added" /
   "Community-checked" badges (unified yellow family — one token per theme) and the map legend are pure renders of
   the DTO's `reviewStatus`/`openStatus`/`nonexistentReports`).
...
- V9 (in `ShelterReportServiceTest`, `ShelterQueryServiceTest`, `ReportThrottleIT` + the
  ...
  (no re-hide after a manual restore / disarmed flag); the `openStatus` derivation (latest fresh state wins, deterministic tie-break); occupancy latest-band-wins + agreeing count + 2 h freshness
...
- V11 community-review-queue (in `CommunityReviewIT` + `ShelterReportServiceTest` +
  `AdminModerationServiceTest`): a new USER row is public as `NEW`; two distinct non-submitter
  confirmations leave it NEW, the third distinct one (open `OPEN_CONFIRMED` reports and current
  OPEN taps, each counted once) promotes it to `CONFIRMED` + `AUTO_CONFIRM` audit row
  (in-transaction, the crossing user as actor);
  the submitter's own positive action does NOT promote; admin CONFIRM promotes; REJECT hides
