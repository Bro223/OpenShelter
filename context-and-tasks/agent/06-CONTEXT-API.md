...
- **Trust filters on the public list (V9, D5; M11 rating demotion):** `GET /api/shelters`
  accepts optional `reviewed` (bool — keeps shelters with at least one VISIBLE review;
  `false` is the negation) and `hasCapacity` (bool — capacity data present; `false` is the
  ...
  Both are applied **in-memory over the already-fetched projection** (Estonia-scale data;
...
- **Admin moderation (`/admin/*`, admin-moderation D3/D4)** — the trust layer's human lever.
  ...
  endpoints. The eighteen endpoints:
...
- V9 (in `ShelterReportServiceTest`, `ShelterQueryServiceTest`, `JpaReportActionLogTest` + the
  ...
  (stale → silent); `reviewed`/`hasCapacity` filter semantics incl. visible-only review
  counting (M11: the minRating filter is gone — the stray-param ignore is pinned in
  `ShelterReportIT`); the 10-active-shelter cap (409; ADMIN kind exempt; deleting or
