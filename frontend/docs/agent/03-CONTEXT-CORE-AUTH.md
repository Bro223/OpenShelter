...
| `AuthStore` | service | session state: `user`-relevant signals (`authenticated`, `levels: VerificationLevel[]`), `init()` (silent refresh at boot), `register/login/logout`, `refresh(): Promise<boolean>` **single-flight** (concurrent callers share one in-flight refresh). Exposes `isVerified()` (any level) used by `VerifiedGuard` and banners. |
...
- M3 builds `VerifyPage`/`ContactChangePage` on `AuthStore.levels()` and the verify/account
  gateways (03 puml).
