...
interface ShelterDto {
  ...
  provenance:
    ...
    | 'UNDER_REVIEW'
    | 'REPORTED_INACTIVE'
    | 'REJECTED'; // server-derived on every
...
interface CommunityPulse {
  ...
  recentReports: {
    ...
    reportedAt: string;
  }[]; // capped at 10, newest first, NO reporter identity
}
...
interface AdminShelterDto {
  ...
  provenance:
    ...
    | 'UNDER_REVIEW'
    | 'REPORTED_INACTIVE'
    | 'REJECTED'; // the same server-derived value as on ShelterDto — the admin list is the one surface where all six values are reachable (it keeps hidden rows)
  inaccurate: boolean;
