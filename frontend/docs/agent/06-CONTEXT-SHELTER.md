...
- **Trust reports** (verified users; the report answers the dampening outcome, the occupancy
  upsert returns 204): **"Report this shelter"** (five types —
  "It does not exist" / "It is closed" / "It is open" / "The location is wrong" /
  "Something else" + optional detail for OTHER) and **"Report how full"** (three large band
  ...
  layer (auto-hide on the 5th non-existence report, the CONFIRMED promotion on an
  OPEN_CONFIRMED report from a non-submitter, display-only occupancy) — the UI renders what
...
7. **Provenance is displayed plainly, not just colour-coded** (accessibility-and-provenance
