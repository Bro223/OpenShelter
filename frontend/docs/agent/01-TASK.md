...
- **moderate (admin only)**: the env-provisioned admin (admin-moderation) sees an admin-only
  "Admin" nav item and the `/admin` page — three tabs working the trust layer: user-shelter
  hide/restore/delete (registry rows are read-only), the shelter-report queue (dismiss), and
  the review-report queue (hide/restore). Regular users never see the nav item, and visiting
  `/admin` redirects them home; the backend re-checks the admin kind per request.

The backend (Spring Boot, same repo, `src/`) is **complete and green (464 tests, counted
2026-09-12)**. This task pack covers the **frontend only**.
...
| `gateways/` `AuthGateway`, `VerifyGateway`, `ShelterGateway` (`list`/`get`/`create`/`mine`/`update`/`remove`/`report`/`reportOccupancy`), `ReviewGateway` (`list`/`add`/`updateMine`/`deleteMine`/`reportReview`), `AccountGateway` (`me`/`updateProfile`/contact-change/`myReviews`), `AdminGateway` (admin-moderation — all eight `/admin/*` endpoints: `listShelters(filters?)`/`setShelterStatus(id, status)`/`deleteShelter(id)`/`listShelterReports(shelterId?)`/`dismissShelterReport(id)`/`listReviewReports()`/`hideReview(reviewId)`/`restoreReview(reviewId)` — the review hide/restore take the REVIEW's id, i.e. `row.reviewId`, not the report row's), `GeoGateway` (`resolve` — POST /api/geo/resolve, short links), `GeocodeGateway` (`search` — client-side OSM Nominatim; the ONE documented raw-fetch exception to the ApiClient rule) — one per backend controller group + the one external-service door `01` |
...
| `features/admin/` (admin-moderation) `AdminPage` — the `/admin` moderation tool (own folder — the no-cross-feature-imports rule holds): route `/admin` (lazy for the bundle budget; `AdminGuard` — anonymous AND non-admin both redirect home), three tabs, each a table with 48px action targets on the existing tokens: **Shelters** — every row incl. hidden (USER rows actionable: Hide/Activate, Delete with a two-tap inline confirm; registry rows read-only — the UI never offers actions for them), columns name/source/status/rating/reviews/reports/occupancy/submitter + a name/address search box (submit-on-enter, server-side match); **Shelter reports** — the queue (shelter link, type, reporter name+email, age, dismiss; dismissed rows stay, dimmed — the audit trail; rows whose shelter is INACTIVE get the "Restore shelter" shortcut); **Review reports** — the queue (shelter, review excerpt with stars + hidden badge, reason, reporter(s), Hide/Restore — the action targets `row.reviewId`). Mutations patch the in-memory row (the backend answers 204 with no body); a rejected mutation surfaces the server message through the page-level banner (403/409 echo the backend message) `01` |
