...
- **contribute**: verified users submit shelters and report them — the typed shelter report
  ...
  and how full a shelter is right now (the community trust layer — the
  trust-weighted "does not exist" tally reaching 5 points auto-hides the shelter
  from the public map, open/closed and occupancy are
...
- **moderate (admin only)**: the env-provisioned admin (admin-moderation) sees an admin-only
  "Admin" nav item and the `/admin` page — nine tabs working the trust layer: the unconfirmed
  ...
  the throttle-abuse alerts (read-only), the user list (suspend/unsuspend), the guidance
  authoring, the media library and the settings tabs, and the audit trail (read-only). Regular users never see the nav item, and visiting
...
- **SCSS** + small design-token file (CSS custom properties — the single `--color-cta`
  ...
  `[data-theme='high-contrast']` override block on `<html>` (the accessibility dialog's
  three-option contrast panel — Default / High contrast / Black and yellow — `aria-pressed`,
  localStorage key `openshelter-theme`, applied pre-paint by the inline `index.html` script —
  accessibility-and-provenance D1/D2; same token names, values differ by theme, default is
  light; the black-and-yellow theme has NO SCSS block — its token values are applied at
  runtime by `ThemeStore`/`theme-tokens.ts` and its page-wide rules ship in
  `shared/accessibility-dialog.component.scss`; the HC block's contrast is audit-verified —
  every text pair ≥ 4.5:1, asserted in
...
- Dev server on **port 5173**; API base URL from `environment.development.ts` —
  same-origin (`''`) in dev: the dev server runs with `--proxy-config proxy.conf.js`
...
| `features/map/` `MapPage` (public map: markers + sidebar list, filter chips, legend; crisis actions — the "Nearest shelter" CTA (`--color-cta`, geolocation + Haversine nearest over the loaded list, per-error copy, row emphasis) and the authenticated-only "Add shelter" → /submit; the Leaflet wrapper lives in `shared/` now; sidebar rows carry the source/trust badge (`sourceTrustLabel` — registry rows name their registry, USER rows their trust state); trust filters — the `Open` / `Has capacity` chips (`Has capacity` a server-side refetch, `Open` a client-side filter over the loaded list) — the source chips are GONE (wave 8: the page fetches `ALL` sources, the legend is the pin-tone filter); reported shelters (either open report kind, W2-A) get the single orange marker + the "Reported" legend entry (`--color-reported`), overriding the trust colour; row badges for the trust state — the "Reported (n)" badge, the open-status badge text, the occupancy text) `01`+`04` |
| `features/admin/` (admin-moderation) `AdminPage` — the `/admin` moderation tool (own folder — the no-cross-feature-imports rule holds): route `/admin` (lazy for the bundle budget; `AdminGuard` — anonymous AND non-admin both redirect home), nine tabs, each a table with 48px action targets on the existing tokens: **Unconfirmed** (first, default) — the unconfirmed queue (the confirm/reject gate for user submissions): every USER row in the NEW state (client-side filter of the shelters list, newest first), "Mark confirmed" direct, "Reject" with a required reason (≤500 chars); **Shelters** — every row incl. hidden (USER rows actionable: Hide/Activate, Delete with a two-tap inline confirm; registry rows read-only — the UI never offers actions for them), columns name/source/status/reports/occupancy/submitter + a name/address search box (submit-on-enter, server-side match); **Shelter reports** — the queue (shelter link, type, reporter name+email, age, dismiss; dismissed rows stay, dimmed — the audit trail; rows whose shelter is INACTIVE get the "Restore shelter" shortcut); **Alerts** — the throttle-abuse ring (daily submission cap, OTP contact cap, near-duplicate rejection; read-only, newest first); **Users** — the account list (name/e-mail/kind/status; Suspend is two-tap and idempotent, never offered for admin-kind rows); **Guidance** — the crisis-guidance authoring (post CRUD, publish/unpublish, ordering, per-locale translations); **Media library** — the image-asset upload/list/delete; **Settings** — the site-text edits; **Audit** (last) — the read-only moderation trail (newest 100, lazy load: when/moderator/shelter/action/change/reason). Mutations patch the in-memory row (the backend answers 204 with no body); a rejected mutation surfaces the server message through the page-level banner (403/409 echo the backend message) `01` |
| `features/shelter/` `ShelterDetailPage` (header carries the coordinate line + the "Navigate" / "Open in Apple Maps" deep links; header badge = the source/trust badge (`sourceTrustLabel`) + the trust badges — the "Reported (n)" badge, the open-status badge, occupancy; three verified-only report sections: "Report this shelter" (three radio types — the negative types, optional detail for the factual ones), "Report open/closed" (two large state buttons, one tap, latest-wins, pre-selected from the detail DTO's `yourOpenStatus`), and "Report how full" (three large band buttons, pre-selected from the detail DTO's `yourOccupancyBand`, aggregate + recency line while fresh)), `SubmitShelterPage` `01`+`05` |
| `shared/` `PageShell` (brand + nav + shell actions, incl. the high-contrast theme toggle bound to `ThemeStore` — and the admin-only "Admin" nav item, rendered only when `AuthStore.isAdmin()` is true, so regular users see the nav unchanged; a failed profile fetch leaves `isAdmin` false, so the item hides itself fail-closed), `BannerComponent`, `LoadingIndicator` (a REAL component now, `role=status`, tokens only), `LeafletService` — plus non-component helpers `form-helpers.ts` (readCoordinate, capacity/name validators, `CODE_SIX_DIGITS`), `shelter-copy.ts` (canonical source copy + `sourceTrustLabel`/`communityTrustLabel`/`communityBadgeClass` — the single-sourced source/trust badge: the registry rows name their registry ("Päästeamet registry" / "Municipal registry"), the USER rows carry the trust-state label ("Newly added" / "Community-checked", and "Rejected" on the /mine + admin surfaces); the legend/filter chip wording is the plain `map.filter.*` copy — plus the trust copy: `openStatusBadgeText` ("Reported closed" hedged at one fresh report / "Closed" firm at two+; a fresh OPEN row carries no badge), `reportedBadgeText` ("Reported (n)" — n = the open trust-report total, the W2-A OR), the occupancy firm/hedged band copy, `occupancyText` (firm or hedged + recency), the two-line detail meta (`lastVerifiedText` + `communityReportsText`), `hasReports`/`hasTrustBadges` — the map rows and the detail header render the SAME badge text) and `location-input.ts` (pure location-string parser: `parseLocationInput`, `isGooShortLink`, `normalizeShortLinkUrl` — fixture-table-tested, no Angular imports) `01` |
