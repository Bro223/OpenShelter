...
export const RU: Messages = {
  ...
  'account.contrib.estoniaNote':
    'Местоположение должно находиться в Эстонии. Проверка выполняется на сервере.',
  ...
  'legal.privacy.why.p2':
    'Правовое основание для каждой цели — [LEGAL BASIS TO BE CONFIRMED]. Настоящий документ призван описывать обработку; он не является юридическим заключением.',
  ...
  'legal.privacy.cookies.p2':
    'Ваш токен доступа удерживается только в памяти и уничтожается, когда вы закрываете вкладку. Ни одна третья сторона не получает эти элементы, и не существует необязательных аналитических или отслеживающих технологий, которые следовало бы принимать или отклонять.',
  ...
  'legal.privacy.retention.p3.before':
    'Эти сроки — правило хранения приложения. Планируемое задание, которое их исполняет, — это переключатель на уровне развёртывания (',
  'legal.privacy.retention.p3.code': 'RETENTION_ENABLED',
  'legal.privacy.retention.p3.after':
    '): в конфигурации разработки этого репозитория он выключен, а включает его тот, кто эксплуатирует развёртывание. В развёртывании, где задание выключено, неактивные аккаунты и старые записи аудита просто сохраняются.',
  ...
  'legal.privacy.security.p2':
    'Приложение применяет ограничения на коды подтверждения, сброс пароля и добавление содержимого, отправляет заголовки безопасности в каждом ответе и требует HTTPS для доступа к местоположению. Ни одна мера безопасности не гарантирует абсолютной безопасности, но эти меры снижают типовые риски.',
  ...
  'legal.terms.rules.li4':
    'Приложение применяет ограничения, чтобы список оставался пригодным для использования: дневной лимит на добавления, лимит на частоту запросов одноразовых кодов и обнаружение близких дубликатов. Превышение лимита приводит к ошибке и рекомендованному времени ожидания; это не блокировка.',
  ...
  'legal.terms.liability.p1':
    'В той мере, в какой это допускается законом, OpenShelter не несёт ответственности за решения, принятые на основе списка. Настоящий пункт предназначен, чтобы быть разумным, и подлежит юридическому рассмотрению; он не пытается исключить ответственность, которую по закону нельзя исключить.',
