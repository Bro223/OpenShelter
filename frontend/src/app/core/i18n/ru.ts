...
export const RU: Messages = {
  ...
  'consent.aria': 'Уведомление о файлах cookie и хранилище браузера',
  ...
  /** Подпись поля импорта по URL (guidance-hero-import): ОЖИДАЮЩИЙ
   *  импорт, сохраняемый с черновиком; сервер скачает его при
   *  публикации. */
  'admin.guidance.editor.hero.importLabel': 'Импорт по URL (необязательно)',
  /** Постоянная подсказка: изображение не скачивается при
   *  редактировании — сервер скачает, проверит и сохранит его при
   *  публикации поста. */
  'admin.guidance.editor.hero.importHint':
    'Изображение не скачивается при редактировании: URL сохраняется вместе с черновиком, а сервер скачает, проверит и сохранит его при публикации поста.',
  ...
  /** Показывается, пока сохранён ожидающий URL импорта: скачивание
   *  происходит при публикации, а неуспешное скачивание прерывает
   *  публикацию (черновик сохраняет URL). */
  'admin.guidance.editor.hero.importNote':
    'Пока ничего не скачано. При публикации сервер скачает изображение — если скачивание не удастся, публикация не состоится и этот черновик сохранит URL.',
  ...
  'legal.privacy.who.p1': 'OpenShelter — карта укрытий и безопасных мест в Эстонии с открытым исходным кодом, поддерживаемая сообществом. Оператор: [OPERATOR LEGAL NAME], [POSTAL ADDRESS]. Контакт по защите данных: [DATA PROTECTION CONTACT].',
  'legal.privacy.who.p2.before': 'OpenShelter ',
  'legal.privacy.who.p2.strong': 'не является официальным государственным сервисом',
  'legal.privacy.who.p2.after': ' и не является аварийной службой. Официальные данные об укрытиях, отображаемые в приложении, импортируются из открытых данных Спасательного департамента (Päästeamet), при этом само приложение управляется независимо.',
  ...
  'legal.privacy.collect.p2.middle': 'собираем национальный код личности и не используем рекламу, аналитику и межсайтовое отслеживание. Когда вы вносите вклад в карту, мы храним содержимое, которое вы добавляете (укрытия и сообщения), как описано в разделе ',
  ...
  'legal.privacy.why.li1.after': ' - отображается на вашем аккаунте и, для публичных добавлений, на карте.',
  ...
  'legal.privacy.verification.p1': 'Картой можно пользоваться без аккаунта. Чтобы добавлять укрытия или сообщения, нужно создать аккаунт и подтвердить одноразовыми кодами и адрес e-mail, и номер телефона. Подтверждение работает так: на каждый контакт отправляется одноразовый код; пока оба не подтверждены, вы можете входить, но не можете добавлять содержимое.',
  ...
  'legal.privacy.verification.p3': 'Для предотвращения злоупотреблений приложение применяет ограничения на запросы кодов и на добавление укрытий и обнаруживает близкие дубликаты. Превышение ограничения приводит к ошибке, а не к блокировке.',
  ...
  'legal.privacy.location.p2.after': '; ваше фактическое местоположение никогда не отправляется на наши серверы. Если вы добавляете укрытие в своём местоположении, сохраняется только выбранная вами координата как часть этого добавления.',
  ...
  'legal.privacy.content.p1.before': 'Когда вы вносите вклад, приложение хранит ваши укрытия и ваши сообщения (например, что место закрыто, указано неточно или больше не существует). Это содержимое становится частью публичной карты сообщества. Свои укрытия вы можете редактировать или удалять ',
  'legal.privacy.content.p1.after': '; сообщения рассматриваются администраторами.',
  'legal.privacy.content.p2': 'Модераторы могут рассматривать, скрывать, исправлять или удалять содержимое, добавленное пользователями. Приложение хранит запись о модерации, чтобы решения можно было проверить.',
  ...
  'legal.privacy.thirdParties.p2': 'Официальные данные об укрытиях импортируются из открытых данных Спасательного департамента (Päästeamet); это входящий источник данных, а не сервис, на который мы отправляем ваши данные. Влечёт ли какой-либо из этих поставщиков международную передачу данных — [TO BE CONFIRMED].',
  ...
  'legal.privacy.retention.p1': 'Данные вашего аккаунта хранятся, пока существует ваш аккаунт. Удаление аккаунта немедленно удаляет ваши персональные данные: укрытия, которые вы указали как частное жильё, удаляются, а публичные укрытия, которые вы добавили, остаются на карте без указания отправителя.',
  ...
  'legal.privacy.retention.p4': 'Публичные добавления сообщества никогда не удаляются автоматически: они остаются на карте без указания автора, пока модератор их не удалит.',
  ...
  'legal.privacy.rights.li1.middle': ' - загрузите JSON-экспорт вашего профиля и всего, что вы добавили, с ',
  ...
  'legal.privacy.rights.li3.after': '. Публичные добавления не удаляются, а остаются на карте без указания автора, как описано выше.',
  ...
  'legal.privacy.security.p2': 'Приложение применяет ограничения на коды подтверждения, сброс пароля и добавление содержимого, отправляет заголовки безопасности в каждом ответе и требует HTTPS для доступа к местоположению. Ни одна мера безопасности не гарантирует абсолютной безопасности, но эти меры снижают типовые риски.',
  ...
  'legal.terms.service.p1': 'OpenShelter — независимая карта укрытий и безопасных мест в Эстонии, поддерживаемая сообществом. Она сочетает официальные открытые данные Спасательного департамента (Päästeamet) с местами, добавленными членами сообщества.',
  ...
  'legal.terms.eligibility.p1': 'Картой можно пользоваться без аккаунта. Чтобы добавлять укрытия или сообщения, нужен аккаунт, и аккаунт становится способным к добавлениям после того, как вы подтвердите одноразовыми кодами и адрес e-mail, и номер телефона. Вы несёте ответственность за точность зарегистрированных контактов.',
  ...
  'legal.terms.rules.li1': 'Добавляйте только те места, о существовании которых вы знаете, с деталями, точными, насколько вам известно.',
  ...
  'legal.terms.rules.li3': 'Не добавляйте частные жилые дома как публичные укрытия. Если вы добавляете место, которое является частным жильём, укажите это.',
  'legal.terms.rules.li4': 'Приложение применяет ограничения, чтобы список оставался пригодным для использования: дневной лимит на добавления, лимит на частоту запросов одноразовых кодов и обнаружение близких дубликатов. Превышение лимита приводит к ошибке и рекомендованному времени ожидания; это не блокировка.',

  'legal.terms.prohibited.p1': 'Вы не должны:',
  'legal.terms.prohibited.li1': 'добавлять ложные, вводящие в заблуждение, небезопасные или вредоносные данные об укрытиях или сообщения;',
  'legal.terms.prohibited.li2': 'добавлять частные жилые дома как публичные укрытия, не указав их как частные;',
  ...
  'legal.terms.prohibited.li5': 'добавлять содержимое, которое является незаконным, порочащим или раскрывающим чужие личные данные.',
  'legal.terms.prohibited.p2': 'Намеренно ложные или вводящие в заблуждение данные об укрытиях — это злоупотребление сервисом, которое может привести к удалению содержимого или приостановке вашего аккаунта.',
  'legal.terms.license.p1': 'Добавляя укрытие или сообщение, вы предоставляете OpenShelter неисключительную, мировую, бесплатную лицензию хранить, отображать и изменять это содержимое в целях эксплуатации карты и его модерации. Право собственности на добавленное остаётся за вами, и вы можете редактировать или удалять свои укрытия.',
  'legal.terms.moderation.p1': 'Добавления попадают в список как сообщения сообщества. Модераторы могут рассматривать, скрывать, исправлять или удалять содержимое, добавленное пользователями, и могут приостанавливать аккаунты, которые злоупотребляют сервисом. Таким образом, добавление может быть рассмотрено, скрыто или отклонено.',
  'legal.terms.official.p1.before': 'Приложение различает источники информации. Места, отмеченные как «Реестр», — из официальных открытых данных. Места, отмеченные «Новое от сообщества» или «Подтверждено сообществом», — добавленные членами сообщества. ',
  'legal.terms.official.p1.em': 'подтверждённый пользователь',
  'legal.terms.official.p1.middle': 'подтвердил владение адресом e-mail и номером телефона; это ничего не говорит о точности того, что он добавляет. ',
  'legal.terms.official.p1.strong': 'Подтверждённый пользователь — не подтверждённое укрытие.',
  'legal.terms.official.p2': 'Место, добавленное сообществом, не является автоматически безопасным, законным, доступным, публичным или действующим укрытием. Относитесь к добавлениям сообщества с осторожностью, особенно во время чрезвычайной ситуации.',
  ...
  'legal.terms.source.p1': 'Исходный код OpenShelter доступен по лицензии MIT. Она регулирует исходный код, но не данные об укрытиях, которые остаются подчинёнными собственным источникам и настоящим условиям.',
