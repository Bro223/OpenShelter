 ...
 *
 *  - request(level)  -> 202 empty  | 409 already verified | 429 cooldown/cap
...
export class VerifyGateway {
  ...
  /** POST /verify/request {level} -> 202. Sends a code via the level's channel. */
  request(level: VerificationLevel): Promise<void> {
    const body: VerifyRequest = { level };
    return lastValueFrom(this.api.post<void>('/verify/request', body));
  }
