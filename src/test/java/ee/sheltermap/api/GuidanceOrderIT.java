 ...
 * rows are cleaned up per test — scoped to THIS class' own rows only (the
 * base cleanup contract), so the other contexts' provisioned admin rows
 * and every other IT's fixtures survive untouched.
 */
...
class GuidanceOrderIT extends AbstractPersistenceIT {
    ...
    void cleanUpCommittedRows() {
        ...
        // first: the translation FK), plus the GUIDANCE_REORDER audit rows,
        // which carry no FK to the posts (shelter_id is FK-less by design
        // and the label-based rows use a dummy shelter id) and would
        // otherwise leak. No other class' rows can match this admin's id.
