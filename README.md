...
- **Crisis guidance (crisis-guidance)** — admin-authored crisis content the public reads
  without an account: the `/blog` index (published posts only, pinned first) and the
  ...
  delete-with-confirm, the create/edit form with the hero picker and the mandatory-alt
...
- PostgreSQL 16 (Docker Compose) · Flyway migrations `V1`–`V24` (SQL, `src/main/resources/db/migration/`) — plus the index-only dotted `V23.1__shelter_bbox_index.sql` (the composite B-tree on the shelter coordinate columns; no PostGIS) — and the Java-based `V13` (`V13PiiEncryptionMigration` — PII-at-rest encryption + blind-index backfill, M2)
...
| GET    | `/api/guidance`                                                                           | public                  | Crisis-guidance index — **published posts only**, pinned posts first and then newest published → `GuidancePostDto[]`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| GET    | `/admin/guidance`                                                                         | JWT + ADMIN kind        | **Every post incl. drafts** → `AdminGuidancePostDto[]`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
- Persistence (Flyway `V1`–`V24` + the Java `V13`, JPA, `ddl-auto=validate`), uniform error handling
