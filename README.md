...
- **Admin moderation (admin-moderation)** — the env-provisioned admin + moderation API:
  ...
  **788 backend tests / 953 frontend tests** (counted 2026-09-15 on this tree;
...
- **Trust & reports (shelter-trust-and-reports)** — community trust layer: shelter
  ...
  **657 frontend tests across 35 spec files** (both counted 2026-09-11 — historical; the
  tree now runs 788 backend / 953 frontend, counted 2026-09-15 above), all green.
...
- PostgreSQL 16 (Docker Compose) · Flyway migrations `V1`–`V23` (SQL, `src/main/resources/db/migration/`) plus the Java-based `V13` (`V13PiiEncryptionMigration` — PII-at-rest encryption + blind-index backfill, M2)
...
| GET    | `/api/guidance`                            | public                  | Crisis-guidance index — **published posts only**, newest first → `GuidancePostDto[]`                                                                                                                                                                                                                                                                                                                                                                                                   |
...
# 3. Run tests (Testcontainers spins its own postgres:16; 788 backend tests, 0 failures —
#    counted 2026-09-15 on this tree with `mvn clean test`)
mvn test
...
7. **Live Twilio send** — the sender, E.164 normalization and the fail-fast at startup are
