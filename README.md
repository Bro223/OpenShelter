...
- PostgreSQL 16 (Docker Compose) · Flyway migrations `V1`–`V35` (SQL, `src/main/resources/db/migration/`) — plus the index-only dotted `V23.1__shelter_bbox_index.sql` (the composite B-tree on the shelter coordinate columns; no PostGIS), the index-only `V30__index_cleanup_and_queue_indexes.sql` (the moderation-actions reporter index + the report-queue ordering index, minus four unused/redundant indexes), the column-only `V31__shelter_submitter_verified_snapshot.sql` (the write-time verification snapshot behind the erasure-stable `submitterVerified`, W2-A part 2), the column-widening `V32__otp_code_hash_keyed_widen.sql` (keyed `v2:` one-time-code hashes are 67 chars, so the OTP columns widen from VARCHAR(64) to VARCHAR(128); refresh tokens stay unkeyed/VARCHAR(64)), the constraint-drop `V33__guidance_hero_import_on_save.sql` (the hero import moved from publish to SAVE — the V25 "no pending import on a published post" CHECK is gone; a page renders the hero only from the stored-asset reference, so the property holds by construction) and the column-only `V35__shelter_submitter_verification_snapshot.sql` (the write-time verification-DEPTH snapshot — the depth-form twin of the V31 boolean; the erasure re-freezes it to the row's last live standing, so an orphaned row keeps the standing it had while the account was active) — and the Java-based `V13` (`V13PiiEncryptionMigration` — PII-at-rest encryption + blind-index backfill, M2) and `V34` (`V34BlindIndexFramingMigration` — length-prefixed blind-index framing + bounded legacy fallback)
...
| Package        | Contents                                                                                                                                                                                                                                                                                                                              |
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `domain`       | `User` hierarchy (incl. `AdminUser` — the env-provisioned admin, admin-moderation), `VerificationClaim`/`VerificationPolicy`/`VerificationRules`, `Shelter`, `Provenance`/`ReviewStatus`, enums, value records — pure Java, no Spring                                                                                                                         |
...
| GET    | `/admin/guidance/{id}/translations`                                                       | JWT + ADMIN kind        | The post's translations in locale order (they feed the public post's alternates list) → `GuidanceTranslationDto[]`; 404 unknown id |
...
#    gates (PMD, the JaCoCo coverage floor) as `mvn verify` against a service
#    Postgres; the OWASP dependency scan (the same `mvn verify` gate) runs on
#    a nightly schedule rather than per push — see .github/workflows/ci.yml.
mvn test
...
repository root lists the variables a real deployment must provide — the database, JWT and PII
credentials, the admin seed, the mail/SMS provider switches and their credentials, and the
dev-only diagnostic toggles — with placeholders only and no real values; copy it and fill in
your own. Every other variable the app reads has a default in
`src/main/resources/application.yml` (the table below covers them all; `application.yml`
is the complete list). Shell-exported env vars take precedence over `.env` values.
...
| `TWILIO_CONNECT_TIMEOUT` / `TWILIO_TIMEOUT`                      | `3s` / `10s`                                                                | Twilio HTTP client timeouts — the send runs on the request thread outside the DB transaction (send-first-then-commit), so a slow provider costs request latency, never a pooled connection                                                                                              |
| `RATELIMIT_TRUST_LOOPBACK`                                       | `true`                                                                      | treat loopback as a trusted reverse proxy for the `X-Forwarded-For` rate-limit keys (local nginx setups) — disable behind a real load balancer where loopback is not your proxy (see `ClientIps`)                                                                                     |
| `MEDIA_DERIVATIVE_MAX_SIDE`                                      | `10000`                                                                     | max image dimension in pixels the derivative generator will decode (a second decompression-bomb guard, independent of the `MEDIA_MAX_BYTES` upload cap)                                                                                                                                                       |
| `HIKARI_MAXIMUM_POOL_SIZE` / `HIKARI_CONNECTION_TIMEOUT`         | `20` / `5000`                                                               | HikariCP pool headroom for the single-instance deployment + the connection timeout that fails a waiting request fast instead of queueing it behind a pinned pool                                                                                                   |
...
- Persistence (Flyway `V1`–`V35` + the Java `V13` + `V34`, JPA, `ddl-auto=validate`), uniform error handling
