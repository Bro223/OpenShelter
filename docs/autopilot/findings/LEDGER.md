...
|---|---|---|---|---|---|---|
| B1 [closed W1 - reconciled by the orchestrator after the wave-1 gate fixes: `AuthRequestConstraintParityTest` now reads the backing FIELD (Jakarta constraints do not target RECORD_COMPONENT) and the focused run is green; the @Size caps themselves landed and were reviewer-verified] | P2 | backend | the 7 auth request records (all lines with `@NotBlank`) | Auth payloads are unbounded while every `api` DTO mirrors its column size; a multi-MB password reaches Argon2 before the per-IP limiter (it counts requests, not bytes) | Add `@Size(max=…)` — email/emailOrPhone 255, password 200, code 16, refreshToken 512 — and a constraint-parity test modelled on `api/ShelterRequestConstraintParityTest` | WS-1 |
...
| F-16 [open W4 - premise DISPROVED by the wave-4 frontend-leftovers lane: the named files dispose a Leaflet map or stop a setInterval countdown, not RxJS subscriptions; the only manual unsubscribe is page-shell.ts:96 (owned by WS-3). Needs a ledger-cell correction, not code.] | P3 | frontend | `features/map/map-page.ts`; `features/shelter/shelter-detail-page.ts`; `features/shelter/submit-shelter-page.ts`; `features/account/verify-page.ts`; `features/account/reset-page.ts` | Manual `ngOnDestroy` unsubscribes everywhere; `takeUntilDestroyed`/`DestroyRef` unused (0 matches) | Migrate the subscriptions in these files as they are touched. `page-shell.ts`, `account-page.ts`, `contributions-panel.ts` are owned by WS-3 → deferred | WS-4 |
...
`openspec/specs/shelter-detail/spec.md`,
`openspec/changes/legal-recovery/tasks.md`, `openspec/specs/shelter-detail/**` (renamed in D21).
...
| D9 [closed W3] | P0 | docs | `frontend/docs/agent/05-CONTEXT-MAP.md:20,28,45,51` | Documents `reviewed`/`minRating` trust filters and a rating select that were removed | Drop both filters; replace the rating sentence with the trust badges the rows actually render (`nonexistentReports`, `openStatus`, `occupancy`, `provenance`) | WS-6 |
...
| D16 [closed W2; caveat: the dropped trust delta file now carries no delta — runner may git rm it if openspec insists] | P2 | openspec | `openspec/specs/shelter-detail/spec.md:5`; `openspec/changes/legal-recovery/tasks.md:13,30` (and its proposal) | The trust change MODIFIES a requirement the removal deleted, so `openspec validate --all` reports "Archive would refuse this delta"; legal-recovery cites the removed `/account/reviews/mine` | Drop the `Community review list` delta; reword the legal-recovery lines to a surviving endpoint | WS-6 |
| D21 [closed W3 - verified done in-tree by the wave-3 reviewer: openspec/specs/shelter-detail/ exists and records the 2026-09-15 rename; the original note asked for a `git mv`, which the orchestrator performed] | P3 | openspec | `openspec/specs/shelter-detail/**` (renamed in D21) → `openspec/specs/shelter-detail/**` | The capability directory still says "reviews" though only the detail page survives (the spec self-documents this follow-up) | `git mv` **after** D16 lands (the archive entry and the trust delta reference the path), then re-run `openspec validate --all` | WS-6 |
...
| H-D22 [DONE - the decision was executed: `README.md` now links both `qa/` and `.agent-orchestration/`] | P3 | docs | `qa/{test-plan.md,feature-matrix.md,security-checklist.md,accessibility-checklist.md}` (the row's original file list omitted `qa/accessibility-checklist.md`, the fourth qa file); `.agent-orchestration/{task-ledger.md,audit-report.md,checkout-checkpoint-log.md}` | Both directories were orphaned (unlinked from README/docs) but not stale, and `qa/` is the only QA coverage map in the repo | Link them from README's documentation index, or move under `docs/`. **Do not delete** — executed as linking: `README.md` now links both `qa/` and `.agent-orchestration/` | HUMAN |
...
- `openspec/specs/*` other than `D21` — clean; `shelter-detail/spec.md` records that removal (the capability was renamed from `shelter-detail-reviews` in D21, now done).
...
- `openspec/changes/archive/2026-09-16-shelter-trust-and-reports` — the change itself is legitimately un-archived; only its stale delta needs `D16`.
...
| ORCH-4 [closed W3] | P3 | test-env | `config/ApiDocsProdClosureIT` (SW-H3) | Passes only because the untracked dev `.env` supplies smtp-pulse/twilio; per the ledger's own caveat it would fail with the documented .env values. | Pin the sender/provider flags in the test profile the way the other config ITs do, so the closure proof does not depend on an untracked file. | WS-7 follow-up |
...
| ORCH-19 | P2 | docs | frontend/docs/agent/00-README.md | Wave-3 reviewer: this file still documents the REMOVED review model and is owned by NO ledger row - the same never-owned-file class as ORCH-1 and ORCH-10. It is the last whole-document instance in the frontend build pack. | Rewrite its review-model references to the shipped surface and grep the pack for the removed identifiers. | next docs wave |
| ORCH-20 | P2 | frontend-a11y | frontend/src/app/shared/page-shell.ts:81-84,100-120 | Wave-3 reviewer, two related behaviours: (a) focus-on-NavigationEnd fires for EVERY NavigationEnd, and because main#main carries tabindex="-1" its focus() also scrolls it into view, so browser back/forward scroll restoration can be overridden; (b) the one-shot firstNavigation boolean means a guard-driven redirect during the initial load (a second NavigationEnd) DOES move focus, so the stated intent 'the browser's focus start must win' silently fails for that second navigation. The two new spec cases assert only 'focus lands' and 'modal keeps focus'. | Decide the intended semantics, narrow the trigger (e.g. skip when the navigation is a back/forward popstate or the initial document load is still settling), and extend ORCH-7's manual pass to cover back/forward + scroll position. | next frontend wave |
| ORCH-21 | P3 | test-env | src/test/java/ee/sheltermap/config/ApiDocsProdClosureIT.java | Wave-3 reviewer: the IT now constructs a real TwilioSmsSender (TwilioSmsSender.java:63-75,121-126 calls Twilio.init with the dummy SID/token). No HTTP at construction and nothing in this IT sends, so it is safe today - but any future assertion added to that context must not expect a live send. | Add a one-line comment in the IT recording the constraint. | next backend wave |

RESOLVED from the same review (no action): R1 (the hand-edited snapshot) - regeneration with -Dopenapi.update=true produced NO diff and OpenApiSnapshotIT is green, so the generator is confirmed as the authority; R5 (no gate ran on the wave) - orchestrator ran mvn clean test (705 green), npm test (953 green) and openspec validate --all (28/28) on the committed wave-3 tree.

## ORCHESTRATOR OWNERSHIP FIX - 2026-09-15T01:03:39Z

Three of the files the reviewers found still describing the removed review model were in the SAME document pack and owned by NO workstream: context-and-tasks/agent/03-CONTEXT-VERIFICATION.md (ORCH-1), the frontend/docs/agent files 01-TASK.md, 06-CONTEXT-SHELTER.md and 07-STEPS.md (ORCH-10), and frontend/docs/agent/00-README.md (ORCH-19). The root cause is that the ledger enumerated WS-6's ownership FILE BY FILE, so any pack file not on that list was invisible to every later sweep.

FIX: the docs workstream's ownership is now the GLOB, not an enumeration - context-and-tasks/agent/**and frontend/docs/agent/** in full, plus context-and-tasks/*.puml and frontend/docs/*.puml. Planners must treat every file in those trees as owned and sweep the whole pack for the removed-feature identifiers (ReviewGateway, RatingStars, ReviewForm, MyReviewDto, AdminReviewReportDto, reviews/mine, shelter_reviews, review_reports, minRating, myReviews) rather than checking an enumerated list.

| ORCH-23 | P2 | openspec | `openspec/changes/archive/2026-09-16-factual-reports-rating-demotion/specs/map-browse/spec.md:11-13,41,47,53` | Wave-4 reviewer: **ORCH-8 only named this change's `shelter-reports` delta**, but the SAME change's `map-browse` delta still specifies `reviewed=true`, the `Reviewed` chip and "the star summary ... remains a READ-ONLY rating display". Archiving that un-archived change would re-introduce removed review/rating text into the BASE `map-browse` spec - the exact class ORCH-12 just fixed for the trust change. | Sweep the WHOLE `factual-reports-rating-demotion` delta set rather than only its `shelter-reports` file, strip the review/rating rules, then `openspec validate --all`. | next openspec wave |

| ORCH-22 | P2 | openspec | openspec/changes/archive/2026-09-16-shelter-trust-and-reports/specs/map-browse/spec.md ; openspec/changes/archive/2026-09-16-shelter-trust-and-reports/specs/shelter-reports/spec.md | ORCH-12's lane reports: with the review/rating text now gone, these deltas are still not archive-clean because they pin a retired field name and shape - the retired display flag `statusFlag` (the 'Closed and open confirmation flag' requirement) versus the live `openStatus` field (state = OPEN/CLOSED). Fixing it needs a spec-ownership decision (the same class of call as the dropped D16 delta), not a mechanical edit. | Decide whether that requirement is superseded by the live open-status model: if yes, drop it the way D16 was dropped (with the rationale in the change's design.md); if no, re-point it at the live field. Until then this change should not be archived. | HUMAN / next openspec wave |
