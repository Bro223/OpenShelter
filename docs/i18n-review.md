...
| `admin.reports.dampened` | `Hõivendatud` | Dampened | WORD CHOICE, not confirmed: the domain sense is "dampened" (a report weighted to zero; RU `Приглушено`). `hõivendatud` is the ordinary Estonian word for damping/attenuating a signal; `vaigistatud` ("muted") is the alternative. The old value `Mõnitatud` ("soothed") was a comfort word, not a signal word. |
