...
| ET | `admin.guidance.order.hint` | `Postid näivad` | `Postid ilmuvad külastajatele…` (two wrong verbs in sequence: the sweep's `näevad` was still wrong — `näha` is transitive, "posts see to visitors"; the intransitive "to appear" is `ilmuma`. See the mechanical-audit table, item 19) |
| ET | `admin.guidance.editor.slugHint.create` | `Tühjaks jättes genereeritakse sluuug` | `Tühjaks jätmisel genereeritakse sluug` (case + misspelling) |
| ET | `admin.guidance.editor.slugHint.edit` | `Tühjaks jättes jääb … sluuug säilima` | `Tühjaks jätmisel jääb … sluug säilma` |
| ET | `admin.guidance.editor.bodyHint` | `…tööriistapaneeli tähised… Liitudud sisu sisestatakse ainult…` | `…tööriistapaneeli vorming… Liidetud sisu hoiab ainult…` (wrong words) |
| ET | `admin.guidance.editor.localeHint` | `Täitub eeltäpselt… muutmisel post ise oma keel` | `Täitub automaatselt… muutmisel posti oma keel` (unworded) |
...
| `admin.reports.notCounted.reason` | `Arvestamata: teatajal on juba olemas sama nimega varjupaik samas asukohas.` | Not counted: the reporter already has a shelter with the same name at the same location. | WORD CHOICE, not confirmed: the possession construction (`teatajal on juba olemas …`) and the `sama nimega … samas asukohas` chain are non-native-speaker guesses; no native speaker has confirmed this sentence. |
...
| `admin.reports.notCounted` | `Не учтено` | Not counted | Badge re-labelled from `Приглушено` ("muted") to plain language — same "jargon communicates nothing" finding as EN/ET. `Не учтено` is the ordinary past-participle phrasing for "not taken into account"; not confirmed by a native speaker. |
| `admin.reports.notCounted.reason` | `Не учтено: у заявителя уже есть своё укрытие с тем же названием в том же месте.` | Not counted: the reporter already has a shelter with the same name at the same location. | WORD CHOICE, not confirmed: `заявитель` (reporter/applicant) chosen over `сообщающий`; the whole sentence is a non-native-speaker judgement, no native confirmation. |
...
- Quoted UI labels ("Näita varjupaiku minu ümbruses", "Kasuta mu asukohta",
  ...
  how.nearest / submit.useMyLocation.
...
## Mechanical dictionary audit (post-sweep) — applied fixes, with certainty levels

A mechanical audit of the ET/RU catalogs (Estonian inflected forms checked
against the Ekilex dictionaries; Russian morphology checked per case and
plural) applied the fixes below directly to
`frontend/src/app/core/i18n/{en,et,ru}.ts` (the EN catalog only for the
quoted-label alignment in item 13). Certainty levels, stated honestly:

- **dictionary-verified** — the replaced form is the dictionary-verified
  form (Ekilex inflection, or unambiguous Russian morphology). Safe to ship;
  a native reviewer may still sanity-check the sentence around it.
- **judgement** — a coherent choice was applied (terminology unification,
  UI-label sync, case parallelism) where the dictionary does not decide;
  the final wording stays with the owner's native reviewer.

Nothing in this section is a native-speaker approval: it records what the
machine did, so the reviewer can tell mechanical fixes from choices. The
"Full listing" tables above show the PRE-audit values; where a row below
changed a listed value, this section wins.

### ET fixes

| # | Key(s) | Before → After | Certainty |
| --- | --- | --- | --- |
| 1 | `legal.terms.rules.li4`, `legal.terms.warranty.p1`, `legal.terms.moderation.p1`, `legal.privacy.sharing.p1`, `legal.terms.liability.p1` | `nimekik`/`Nimekik`/`nimekikku`/`nimekiku`/`nimekikule` (not a word) → `nimekiri`/`Nimekiri`/`nimekirja`/`nimekirja`/`nimekirjale` (the word is `nimekiri`; each occurrence took its dictionary case) | dictionary-verified |
| 2 | `legal.privacy.collect.p2` | `Me ei kogume … ega kasuta …` → `Me ei kogu … ega kasuta …` (1st-person negative takes the 2nd-sg form; the sentence's second verb already had it) | dictionary-verified |
| 3 | `legal.terms.emergency.p2` | `hülgatud hoone` → `hülgatud hoonesse` (illative, parallel with `eraomandisse`) — safety-critical sentence | dictionary-verified |
| 4 | `legal.privacy.cookies`, `legal.privacy.cookies.p1` | `Cookied ja brauseri salvestusruum` → `Küpsised ja brauseri salvestusruum`; `reklaamikooge` → `reklaamiküpsiseid` (the catalog's own consent banner uses `küpsis`) | dictionary-verified |
| 5 | `legal.privacy.retention.p1`, `legal.terms.prohibited.li2` | `eraomana` → `eraomandina`; `eraomanina` → `eraomanikuna` (adessive forms) | dictionary-verified |
| 6 | `legal.privacy.verification.p3` | `mitte keeluse` → `mitte keelust` (partitive after `annab`, parallel with `vea`) | dictionary-verified |
| 7 | `submit.hint.swapped` | `pikkus ja laius` (length/width) → `pikkuskraad ja laiuskraad` (longitude/latitude) | dictionary-verified |
| 8 | `submit.privateLabel`, `legal.terms.rules.li3`, `legal.terms.prohibited.li2` | `privaalaru` (not a word) → `erakodu`; `erakoju` → `erakodu` (one form across the pair) | judgement (the unification choice); `privaalaru` was not a word regardless |
| 9 | `authPage.reset.sentTitle` | `Kontrolli oma sissetulekut` (income) → `Kontrolli oma postkasti` (inbox) | dictionary-verified |
| 10 | `account.delete` | `Konta kustutamine` → `Konto kustutamine` (genitive in the compound) | dictionary-verified |
| 11 | `guidance.title`, `title.guidance` | `Kriisijuhtimine` ("crisis management") → `Kriisijuhised` (the feature's other keys all use `juhis`) | judgement (coherence) |
| 12 | `how.example.1` | `sinise märgise ja sildiga` → `sinise märgisega ja sildiga` (inessive) | dictionary-verified |
| 13 | `how.sources`, `how.example.1`, `how.example.2`, `legal.terms.official.p1.before` (ET + EN) | ET quoted the ENGLISH legend labels ("Registry", "Newly added", "Community-checked") while the ET map legend renders `Register` / `Uus kogukonnalt` / `Kogukonna poolt kinnitatud` → each string now quotes the label the UI actually shows. The EN side had the same mismatch (`how.*`/official quoted "Newly added"/"Community-checked" while the EN legend shows "New by community"/"Confirmed by community"); the EN quotes were aligned first, so the quoted names have one stable target in every locale: `map.legend.*`. RU already quoted its own legend. The stale ET comment claiming the badge quotes keep "the English word + the Estonian gloss" was replaced. | judgement (UI-label sync) |
| 14 | terminology doubles | `panus`/`esitus` → **`esitus`** (`submit.success.viewContributions` `panuseid`→`esitusi`; `legal.privacy.collect.p2.middle` + `legal.privacy.content.p1.before` `panustad`→`esitad`; `legal.terms.rules` `Panuste reeglid`→`Esituste reeglid`); `ristsaidi jälgimist`/`saitidevahelist jälgimist` → **`saitidevaheline`** (`consent.body`); `seansitunnus`/`sisselogimistunnus` → **`sisselogimistunnus`** (`consent.body` — the EN source says "sign-in token" in both places); `juhisepost`/`juhistepost`/`juhiseliste` → **`juhistepost`** (`admin.guidance.loading`, `admin.guidance.empty`, `admin.guidance.editor.createTitle`/`.editTitle`/`.pinnedLabel`, `admin.media.delete.inUse`) | judgement (unification choices) |
| 19 | `admin.guidance.order.hint` | `Postid näevad külastajatele selles järjekorras…` → `Postid ilmuvad külastajatele selles järjekorras…` (the sweep's `näevad` was still wrong: `näha` is TRANSITIVE — "the posts see the visitors"; the intransitive "to appear" is `ilmuma`. The earlier `näivad → näevad` fix replaced one wrong verb with another, which is why it survived to this audit.) | dictionary-verified (transitivity) |
| 20 | `admin.siteTexts.hint1` | `(näidatud kaoaiana)` — "shown for as long as it lasts"; the placeholder concept was gone (`kauge`-family mistranslation of "placeholder") → `(näidatud kaaplasolderina)`. Grep found NO existing Estonian "placeholder" term in the catalog (the `*Placeholder` keys carry sample text only; the only rendered instance of the concept is RU's loanword `плейсхолдер`), so the Estonian loanword `kaaplasolder` mirrors RU. Alternative: `otsetekst`. | judgement (WORD CHOICE — not confirmed by a native speaker) |
| 21 | `admin.reports.dampened` | `Mõnitatud` ("soothed" — a comfort word) → `Hõivendatud` (the ordinary Estonian word for damping/attenuating a signal; RU `Приглушено` is correct). Alternative: `vaigistatud` ("muted"). | judgement (WORD CHOICE — not confirmed by a native speaker) |

### RU fixes

| # | Key(s) | Before → After | Certainty |
| --- | --- | --- | --- |
| 15 | 24 occurrences in 21 legal keys: `legal.privacy.who.p1`, `legal.privacy.who.p2.after`, `legal.privacy.collect.p2.middle`, `legal.privacy.verification.p1`, `legal.privacy.verification.p3`, `legal.privacy.location.p2.before`, `legal.privacy.location.p2.after`, `legal.privacy.content.p1.before`, `legal.privacy.thirdParties.p2`, `legal.privacy.sharing.p1`, `legal.privacy.retention.p1`, `legal.terms.service.p1`, `legal.terms.eligibility.p1`, `legal.terms.rules.li3`, `legal.terms.prohibited.li1`, `legal.terms.prohibited.li2`, `legal.terms.prohibited.p2`, `legal.terms.license.p1`, `legal.terms.official.p1.strong`, `legal.terms.official.p2`, `legal.terms.source.p1` | `убежище` → `укрытие` with case/plural agreement per occurrence (`укрытия`, `укрытий`, `об укрытиях`, `укрытием`, …). The owner confirmed both words are acceptable; the UI already used `укрытие` in ~52 keys (per the audit), so the legal minority changed. The RU header comment records the unification. | judgement (owner-confirmed word); per-occurrence morphology dictionary-verified |
| 16 | `account.contrib.hidden` | `об этом сообщили из сообщества` → `об этом сообщило сообщество` (the agent of the clause needs the instrumental) | dictionary-verified |
| 17 | `consent.aria`, `consent.title` | `Уведомление о cookie и хранилище браузера` / `О cookie и хранилище браузера` → `Уведомление о файлах cookie и хранилище браузера` / `О файлах cookie и хранилище браузера` (matching `legal.privacy.cookies`) | judgement (UI-label sync) |
| 18 | terminology doubles | submit verb `отправлять`/`добавлять` → **`добавлять`** (UI majority: `Добавить укрытие` in `title.submit`, `map.addShelter`, `submit.title`, `submit.submit`, `account.contrib.submit`, `account.contrib.emptyCta`, plus `добавлять укрытия` in `authPage.register.subtitle`, `verify.subtitle`, `verify.fullyVerifiedCopy`, `verify.verifiedCopy`; the legal text and `submit.submitting` were the outliers — now `добавлять`/`добавьте`/`добавляя`/`добавленные`/`добавленное`/`добавили`/`Добавляем…` across `legal.privacy.verification.p1`, `legal.privacy.verification.p3`, `legal.privacy.location.p2.after`, `legal.privacy.why.li5.strong`, `legal.privacy.content.p2`, `legal.privacy.sharing.p1`, `legal.privacy.retention.p1`, `legal.privacy.rights.li1.middle`, `legal.privacy.security.p2`, `legal.terms.service.p1`, `legal.terms.eligibility.p1`, `legal.terms.rules.li1`, `legal.terms.rules.li3`, `legal.terms.prohibited.li1`, `legal.terms.prohibited.li2`, `legal.terms.prohibited.li5`, `legal.terms.license.p1`, `legal.terms.moderation.p1`, `legal.terms.official.p1.before`, `legal.terms.official.p1.middle`, `legal.terms.official.p2`). Contributions noun `добавления`/`публикации` → **`добавления`** (UI: `account.contributions` "Мои добавления", `submit.success.viewContributions` "Посмотреть ваши добавления"; the legal text was the outlier — `legal.privacy.why.li1.after`, `legal.privacy.sharing.p1`, `legal.privacy.retention.p4`, `legal.privacy.rights.li3.after`, `legal.terms.rules`, `legal.terms.rules.li4`, `legal.terms.eligibility.p1`, `legal.terms.moderation.p1`, `legal.terms.official.p2`). Send-sense `отправлять` (codes, SMS, reports, questions, location-to-server) was NOT touched; `форма отправки` (the submit form) was left as the generic UI term; guidance-admin `публикация` (publish) was NOT touched — a different sense. | judgement (unification choices) |

### Still flagged — NOT fixed by the audit

The severity-4 coinages and house-style judgements stay flagged for the
owner's reviewer; the audit must not silently "improve" them:

- ET: `edastatavus` (`legal.privacy.rights.li1.strong2`), `Kasutamisvõimalus`
  (`legal.terms.eligibility`), `tarneteenus`
  (`legal.privacy.thirdParties.li1.strong`), `turvepäised`
  (`legal.privacy.security.p2`), `mitmugi`
  (`legal.privacy.location.p3.strong`).
